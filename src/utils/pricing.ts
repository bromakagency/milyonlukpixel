...
export function getKdvAmountFromNet(netAmount: number): number {
  return Math.round(netAmount * KDV_RATE);
}

export function getGrossPriceFromNet(netAmount: number): number {
  return netAmount + getKdvAmountFromNet(netAmount);
}

export function getGrossPriceFromBlocks(w: number, h: number): number {
  return getGrossPriceFromNet(getNetPriceFromBlocks(w, h));
}
